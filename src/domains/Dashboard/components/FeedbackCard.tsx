...
const Feedback: React.FC<FeedbackProps> = ({ feedbacks }) => {
  return (
    <div className="bg-white border rounded-lg shadow-md p-6 w-full h-auto mt-8">
      <div className='flex justify-between px-3'>
        <span className='mb-4'>
          <h2 className="text-lg font-semibold mb-2">Recent Feedback</h2>
          <h3 className='text-gray-500 text-sm '>Feedback that need to be approved based on fact</h3>
        </span>
        <span>
          <Link to={""} className='text-blue-600 flex items-center text-sm whitespace-nowrap'>
      ...
      {feedbacks.map((item, index) => (
        <div key={index} className="border p-7 rounded-lg mb-4">
          ...
          <Link to={""} className="text-blue-500 text-sm flex items-center whitespace-nowrap">View Photos <FaGreaterThan  size={12}  className='ml-2' /></Link>
        </div>
      ))}
    </div>
  );
